...
Point bunnyLocation[NUM_OF_BUNNIES];
...
void display(void)
{
    ...
    if (defaultCamera) {
        ...
        glMatrixMode(GL_PROJECTION);
        glLoadIdentity();
        gluPerspective(100, (GLfloat)800 / (GLfloat)800, SPEED, 100.0);
        glMatrixMode(GL_MODELVIEW);
        glLoadIdentity();
        Point dir = diff(snakeBody[(bodySegments + firstIndex) % (MAX_SEGMENTS)],
            snakeBody[(firstIndex + bodySegments - 1) % (MAX_SEGMENTS)]);
        cameraPos(cameraX, cameraY, cameraZ, dir);
        gluLookAt(
            snakeBody[(bodySegments + firstIndex) % MAX_SEGMENTS].x,
            snakeBody[(bodySegments + firstIndex) % MAX_SEGMENTS].y,
            snakeBody[(bodySegments + firstIndex) % MAX_SEGMENTS].z,
