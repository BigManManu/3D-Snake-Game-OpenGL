...
void Shapes::eyeBall(double rx, double ry, double rz, int lats, int longs, ColorPattern color, ColorPattern frontColor, bool orientation) 
{
    int i, j;
    for (i = 0; i <= lats; i++) {
        ...
        for (j = 0; j <= longs; j++) {
            ...
            double x = cos(theta);
            double y = sin(theta);
            if (phi0 < 0 && orientation || phi0 >= 0 && !orientation)
                switch (color) {
                case CHECKERED:
...
void Shapes::head(double rx, double r, int lats, int longs, ColorPattern color)
{
    int i, j;
    for (i = 0; i <= lats; i++) {
        double phi0 = M_PI * (-0.5 + (double)(i - 1) / lats);
    ...
    eyeBall(r / 2, r / 4, r / 5, lats, longs, color, SOLID_WHITE, false);
    glPushMatrix();
    glTranslatef(r / 10, 0, 0);
    eyeBall(r / 4, r / 5, r / 5, lats, longs, color, SOLID_BLACK, false);
    glPopMatrix();
    glPopMatrix();
    glPushMatrix();
    glTranslatef(rx / 2, -r / 3, r / -1.3);
...
void Shapes::cone(int lats, int longs, double r1, double r2, double L, ColorPattern color)
...
void Shapes::torus(int lats, int longs, double fromTheta, double toTheta, double r1, double r2,
