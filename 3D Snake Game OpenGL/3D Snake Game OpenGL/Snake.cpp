...
void Snake::render(double r, double tailR, ColorPattern color, Point snakeUp)
{
    Point d, nextD;
    double direction;
    double l = length(r);
    ...
    for (int i = 1; i <= bodySegments; i++) {
        ...
        if (i == 1) {//tail
            ...
            if (tailLength > 0) {
                ...
                shape.cone(lats, longs * tailLength, direction > 0 ? 0 : tailR * tailLength / 2,
        ...
        if (l0 < 0) {//i = 2
            ...
            shape.cone(lats, longs * tailRes, direction > 0 ? r2 : tailR * r,
        ...
        if (dir > 4 * r) {
            ...
            if (direction < 0)
                shape.cone(lats, longs * temp, r1, tailR * r + (1 - tailR) * r * l0 / l, temp, color);
            else
                shape.cone(lats, longs * temp, tailR * r + (1 - tailR) * r * l0 / l, r1, temp, color);
        ...
        if (i == bodySegments && !headRotation) {
            glPushMatrix();
            shape.head(direction > 0 ? 2 * r : -2 * r, r, lats, longs * M_PI * r, color);
            glPopMatrix();
        }
        else if (i == bodySegments && headRotation)
        {
            ...
            if (d.x == 0 && nextD.x == 0) {//in y-z plane
                if (d.z == 0) {//y-->z
                    ...
                    shape.torus(lats, longs * M_PI * r, torusAngle, d.y > 0 ? 0 : 2 * torusAngle, r1, r2, 2 * r, color, r0, tailRemainder, arc / (2 * r), headAngle, 2 * r);
                }
                else {//z-->y
                    glTranslatef(0, nextD.y > 0 ? 2 * r : -2 * r, 0);
                    glRotatef(-90, 0, 0, 1);
                    double torusAngle = d.z > 0 ? M_PI / 2 : -M_PI / 2;
                    shape.torus(lats, longs * M_PI * r, nextD.y > 0 ? 0 : 2 * torusAngle, torusAngle, r1, r2, 2 * r, color, r0, tailRemainder, arc / (2 * r), headAngle, 2 * r);
                }
            }
                ...
                if (d.x == 0) {//z-->x
                    ...
                    shape.torus(lats, longs * M_PI * r, torusAngle, d.z > 0 ? 0 : 2 * torusAngle, r1, r2, 2 * r, color, r0, tailRemainder, arc / (2 * r), headAngle, 2 * r);
                }
                else {//x-->z 
                    glTranslatef(0, 0, nextD.z > 0 ? 2 * r : -2 * r);
                    glRotatef(-90, 1, 0, 0);
                    double torusAngle = nextD.z > 0 ? M_PI / 2 : -M_PI / 2;
                    shape.torus(lats, longs * M_PI * r, torusAngle, d.x > 0 ? 0 : 2 * torusAngle, r1, r2, 2 * r, color, r0, tailRemainder, arc / (2 * r), headAngle, 2 * r);
                }
            }
            else {//in x-y plane
                if (d.y == 0) {//x-->y
...
int Snake::eatsBunny(Point bunnyLocation[], bool &isGoldenBunny, Snake &snake)
{
    int head = (bodySegments + firstIndex) % MAX_SEGMENTS;
    for (int i = 0; i < NUMBER_OF_BUNNIES; i++)
        if (euclideanDistanceSquare(snakeBody[head], bunnyLocation[i]) <= (THICKNESS + BUNNY_SIZE) * (THICKNESS + BUNNY_SIZE)) {
            snake.score++;
            return i;
        }
    if (isGoldenBunny) {
        if (euclideanDistanceSquare(snakeBody[head], bunnyLocation[NUMBER_OF_BUNNIES]) <= (THICKNESS + BUNNY_SIZE) * (THICKNESS + BUNNY_SIZE)) {
            snake.score += 10;
            isGoldenBunny = false;
            return 10;
        }
