...
void display()
{
    ...
    if (defaultCamera) {
        ...
        camera.FirstPersonCamera(snake);
    }
    if (gameover) {
        glColor4f(1, 0, 0, ((double)blinking) / BLINKING_PERIOD);
        Gui::GameHud(0.4, 0.5, (void*)font, "GAME OVER");
        Gui::GameHud(0.42, 0.52, (void*)font_small, "Press R Key to Reset");
    }
    if (!moving) {
        if (playing) {
...
void keyboard(unsigned char key, int x, int y)
{
    ...
    case 'D':
        debug = !debug;
        break;
    case 'r':// Reset game after Game Over
    case 'R':
        if(gameover)
            reset();
        break;
    ...
    case '-'://scaling down the shape uniformly
        ...
        break;
    case 'i':
    case 'I'://Reset to normal view after zooming/rotating camera
        glLoadIdentity();
        gluLookAt(0, 0, EYE_Z, 0, 0, 0, 0, 1, 0);
